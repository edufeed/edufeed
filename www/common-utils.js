...
(function(){
  var isChromeApp, isMobileChromeApp, fixMediaURL, getLocalStorage, getUsername, getPassword, getCouchURL, setUsername, setPassword, setCouchURL, memoizeSingleAsync, onceTrue, getClasses, getBFFs, getUsersBFF, getUsersClass, getAllUsers, getClassmates, errorlog, adderror, geterrors, itemtype_and_data_matches, social_sharing_data, setSocialSharingData, getSocialSharingData, capitalizeFirstLetter, getUrlParameters, getParam, getBoolParam, setParam, parseInlineCSS, applyStyleTo, setPropDict, tagMatchesItem, out$ = typeof exports != 'undefined' && exports || this;
  ...
  out$.getBFFs = getBFFs = memoizeSingleAsync(function(callback){
    return $.get('/bestfriends.yaml', function(yamltxt){
      var data;
      data = jsyaml.safeLoad(yamltxt);
      return callback(data);
    });
  });
  out$.getUsersBFF = getUsersBFF = function(username, callback){
    return getBFFs(function(friends){
      if (in$(username, Object.keys(friends))) {
        if (friends[username] != null) {
